...
<script lang="ts">
  ...
  import { toaster } from "$/shared/lib/toaster-state.svelte";

  import Caption from "./caption.svelte";

  const createToastStyle = cva([
    "flex items-center gap-2",
    "px-3 py-2 max-w-lg rounded-lg",
  ...
  const createToastButtonStyle = cva([
    ...
    "text-sm tracking-tight rounded-lg cursor-pointer",
  ...
  const toastStyle = $derived(createToastStyle());
  const toastButtonStyle = $derived(createToastButtonStyle());
</script>
