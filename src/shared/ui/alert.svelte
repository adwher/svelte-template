...
<script module lang="ts">
  ...
  export const createAlertStyle = cva(["p-4 rounded-lg"], {
    variants: {
      severity: {
        message: ["text-teal-800", "bg-teal-500/10"],
        warning: ["text-orange-600", "bg-orange-300/10"],
        error: ["text-orange-800", "bg-orange-600/10"],
      },
...
<script lang="ts">
  ...
  const alertStyle = $derived(createAlertStyle({ severity, horizontal, class: props.class }));
</script>

<quote {...props} class={alertStyle}>
  {@render children?.()}
</quote>
